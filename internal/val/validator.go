...
func ValidateUsername(value string) error {
	...
	if !isValidUsername(value) {
		return fmt.Errorf("%s is not a valid username, it must contain only letter, numbers and underscore", value)
	}
	return nil
}
...
func ValidateUserFullName(value string) error {
	...
	if !isValidFullName(value) {
		return fmt.Errorf("%s is not a valid full name, it must contain only letter and space", value)
	}
	return nil
}
